#!/usr/bin/env bash
# 🚀 UNIFIED TRADING SYSTEM STARTUP (NO ADMIN REQUIRED)
# Complete deployment without requiring admin privileges
...
echo "🚀 UNIFIED TRADING SYSTEM DEPLOYMENT (NO ADMIN)"
echo "================================================"
echo "Date: $(date)"
...
cleanup_processes "user_watchdog" "User Watchdog processes"
...
try:
    ...
    embeds = [{
        'title': '🚀 Trading System Startup Test (No Admin)',
        ...
        'fields': [
            ...
            {'name': 'Admin Required', 'value': 'NO', 'inline': True}
        ]
    }]
...
try:
    ...
    send_message('🚀 Trading System Startup Test (No Admin) - Phase 1C deployment with ultra-aggressive thresholds')
...
# ============================================================================
# PHASE 6: SETUP USER-SPACE MONITORING (NO ADMIN REQUIRED)
# ============================================================================

echo "🔧 PHASE 6: Setting up user-space monitoring (no admin required)..."

# Create user watchdog script
cat > user_watchdog_monitor.sh << 'EOF'
#!/bin/bash
# User-space watchdog monitor (no admin privileges required)

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ROOT_DIR"

echo "[$(date)] Starting user-space watchdog monitor" >> user_watchdog.log
...
while true; do
    ...
    if ! pgrep -f "python3 scripts/hybrid_crypto_trader.py" >/dev/null 2>&1; then
        echo "[$(date)] Hybrid Agent down, restarting..." >> user_watchdog.log
        bash scripts/start_hybrid_loop.sh &
    fi
    
    # Check Futures Agent  
    if ! pgrep -f "python3.*high_risk_futures_agent.py" >/dev/null 2>&1; then
        echo "[$(date)] Futures Agent down, restarting..." >> user_watchdog.log
        bash scripts/start_futures_loop.sh &
    fi
    
    # Health check every 30 minutes (1800 seconds = 30 minutes)
    if [ $(($(date +%s) % 1800)) -lt 120 ]; then
        echo "[$(date)] Running health check..." >> user_watchdog.log
        if [ -f scripts/health_check.sh ]; then
            bash scripts/health_check.sh >> user_watchdog.log 2>&1 || true
        fi
    fi
    
    # Database cleanup every 6 hours (21600 seconds = 6 hours)
    if [ $(($(date +%s) % 21600)) -lt 120 ]; then
        echo "[$(date)] Running database cleanup..." >> user_watchdog.log
        if [ -f database_cleanup.py ]; then
            python3 database_cleanup.py >> user_watchdog.log 2>&1 || true
        fi
    fi
done
EOF

chmod +x user_watchdog_monitor.sh

# Start the user watchdog in background
echo "   Starting user-space watchdog monitor..."
nohup bash user_watchdog_monitor.sh > user_watchdog.log 2>&1 &
WATCHDOG_PID=$!

echo "✅ User-space monitoring setup complete:"
echo "   - Watchdog checks every 2 minutes (PID: $WATCHDOG_PID)"
echo "   - Health checks every 30 minutes"
echo "   - Database cleanup every 6 hours"
echo "   - No admin privileges required"
echo "   - Logs: user_watchdog.log"
echo
...
# Check watchdog
if pgrep -f "user_watchdog_monitor.sh" >/dev/null 2>&1; then
    WATCHDOG_PID=$(pgrep -f "user_watchdog_monitor.sh" | head -1)
...
# Check logs
echo "✅ Logs: trader_loop.log, high_risk_futures_loop.log, user_watchdog.log"
...
echo "watch 'ps aux | grep -E \"(hybrid|futures|watchdog)\" | grep -v grep'"
echo
echo "# Live logs:"
echo "tail -f trader_loop.log high_risk_futures_loop.log user_watchdog.log"
echo
...
echo
echo "# Emergency stop:"
echo "pkill -f 'python3.*hybrid_crypto_trader.py'; pkill -f 'python3.*high_risk_futures_agent.py'; pkill -f 'user_watchdog_monitor.sh'"

echo
echo "🚀 UNIFIED TRADING SYSTEM DEPLOYMENT COMPLETE! (NO ADMIN REQUIRED)"
echo "=================================================================="
...
echo "✅ User-space monitoring and auto-recovery active (no admin privileges)"
echo "✅ Discord/Telegram notifications configured"
echo "✅ Database logging for performance assessment enabled"
echo "✅ Automated monitoring and cleanup processes"
echo
...
echo "🔄 Auto-recovery on failures (user-space watchdog)"
echo "🛡️  NO ADMIN PRIVILEGES REQUIRED"
...
try:
    ...
    futures_running = subprocess.run(['pgrep', '-f', 'python3.*high_risk_futures_agent.py'], 
                                   capture_output=True).returncode == 0
    watchdog_running = subprocess.run(['pgrep', '-f', 'user_watchdog_monitor.sh'], 
                                    capture_output=True).returncode == 0
    
    status_color = 0x00FF00 if (hybrid_running and futures_running and watchdog_running) else 0xFF6B35
    
    embeds = [{
        'title': '🚀 Unified Trading System Deployed (No Admin)',
        'description': 'Complete trading system deployment with Phase 1C configuration - NO admin privileges required',
        'color': status_color,
        'fields': [
            ...
            {'name': 'User Watchdog', 'value': '✅ RUNNING' if watchdog_running else '❌ FAILED', 'inline': True},
...
echo "🔧 NEXT STEPS:"
echo "=============="
echo "1. Monitor with: tail -f user_watchdog.log"
echo "2. Check status: ps aux | grep -E '(hybrid|futures|watchdog)' | grep -v grep"
echo "3. Stop system: pkill -f 'user_watchdog_monitor.sh'; pkill -f 'python3.*hybrid_crypto_trader.py'; pkill -f 'python3.*high_risk_futures_agent.py'"
echo "4. Restart: ./start_trading_system_no_admin.sh"
