...
# Ensure repository root is on sys.path so local imports work when run as a script
...
def _load_env() -> None:
    ...
    Tries to respect the current working directory first for local dev.
    """
    try:
...
def main() -> int:
    ...
    TB_* envs are intended for automation/testing; avoid hardcoding secrets.
    """
    ...
    # Some components respect TB_*; tracer_bullet checks TB_NO_TELEGRAM internally.
